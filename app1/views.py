...
def index(request):
    ...
    recent_blogs = Blogs.objects.all().order_by("-created_at")[:3]
    default_value = ""
    context={
...
def contact_form(request):
    if request.method == "POST":
        ...
        try:
            ...
            is_error = True
            error_message = str(e)
            messages.error(request, "An error occured while sending the email")
        else:
            is_success = True
...
def blog_detail(request, blog_id):
    ...
    recent_blogs = Blogs.objects.all().exclude(id = blog_id).order_by("-created_at")[:2]
    context ={
        "blog" : blog,
        "recent_blogs" :  recent_blogs
    }
...
def blogs(request):
    all_blogs = Blogs.objects.all().order_by("-created_at")
